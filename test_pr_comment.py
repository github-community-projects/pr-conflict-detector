...
class TestPostPRComments(unittest.TestCase):
    ...
    @patch("pr_comment._find_existing_comment", return_value=None)
    ...
    @patch("pr_comment._find_existing_comment")
...
class TestFindExistingComment(unittest.TestCase):
    """Tests for the _find_existing_comment function."""

    def test_find_existing_comment_found(self):
        """Should return the comment if one with signature exists."""
        repo = MagicMock()
        pr = MagicMock()
        ...
        result = pr_comment._find_existing_comment(repo, 123)
        self.assertEqual(result, comment2)

    def test_find_existing_comment_not_found(self):
        """Should return None if no matching comment exists."""
        repo = MagicMock()
        pr = MagicMock()
        repo.pull_request.return_value = pr
    ...
    def test_find_existing_comment_error_handling(self):
        """Should return None on error to avoid blocking."""
        repo = MagicMock()
